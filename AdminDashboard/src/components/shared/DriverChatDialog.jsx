...
export function DriverChatDialog({ open, onOpenChange, target }) {
      ...
      <DialogContent className="max-w-lg">
        ...
        <ScrollArea className="h-72 rounded border bg-muted/20 px-3 py-2">
          <div className="space-y-3">
            {messages.map((message) => (<div key={message.id} className={`flex ${message.sender === "Dispatch" ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[75%] rounded-lg px-3 py-2 text-sm shadow-sm ${message.sender === "Dispatch"
            ...
            {messages.length === 0 && (<p className="text-sm text-muted-foreground">Start a conversation with the driver.</p>)}
          </div>
        </ScrollArea>

        <DialogFooter className="gap-2">
          <Input placeholder="Send a quick message" value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => {
            if (event.key === "Enter") {
