...
import { useDriversSummary, useOrderStats } from "@/utils/hooks/use-api";

export function Analytics() {
  // API Data
  const { data: driversSummary } = useDriversSummary();
  const { data: orderStats } = useOrderStats();

  // Operational Data - Hardcoded (Requires time-series/zone-level backend routes)
  ...
  // Safety Data - Hardcoded (Requires safety alerts/incident backend routes)
  const fatigueData = [
    { day: "Mon", alerts: 12 },
    { day: "Tue", alerts: 15 },
    { day: "Wed", alerts: 22 },
  ...
  // Predictive Data - Hardcoded (Requires AI integration/forecasting backend routes)
  ...
  // Order Stats
  const avgDeliveryTime = orderStats?.avg_delivery_time_min || 0;
  const activeOrders = orderStats?.assigned || 0 + orderStats?.picked_up || 0;
  ...
  return (<div className="space-y-6 p-6">
    ...
    <Tabs defaultValue="operational" className="space-y-6">
      ...
      <TabsContent value="operational" className="space-y-6">
        {/* Key Metrics */}
        {/*Change and certain values are hardcoded. Needs to be updated */}
        <div className="grid grid-cols-4 gap-4">
          <StatCard title="Avg Delivery Time" value={`${avgDeliveryTime} min`} change="12%" icon={Clock} trend="up" />
          <StatCard title="Active Orders" value={activeOrders} change="+18%" icon={Package} trend="up" />
          <StatCard title="Driver Utilization" value={`${utilization}%`} change="+5%" icon={Users} trend="up" />
          <StatCard title="Pickup Delay" value="0 min" change="+15%" icon={AlertTriangle} trend="down" />
        </div>
      ...
      <TabsContent value="safety" className="space-y-6">
        {/* Safety Metrics */}
        <div className="grid grid-cols-4 gap-4">
          <StatCard title="Total Incidents" value="396" change="+8%" icon={AlertTriangle} trend="down" />
          <StatCard title="Fatigue Alerts" value="145" change="+22%" icon={Activity} trend="down" />
          <StatCard title="Fleet Safety Score" value="89/100" change="+3%" icon={Target} trend="up" />
          <StatCard title="Accident Rate" value="0.8%" change="-15%" icon={Zap} trend="up" />
        </div>
